...
namespace Numantics {
	public class Numantics : ResoniteMod {
		...
		private static readonly ModConfigurationKey<bool> RoundResults =
			new ModConfigurationKey<bool>("round_results", "Round results to nearest integer for numeric fields only (e.g., 5*0.5=3 instead of 2.5)", () => false);
		...
		class TextEditor_OnFinished_Patch {
			static void Prefix(TextEditor __instance) {
				try {
					...
					bool isIntField = fieldType != null && IsIntegerType(fieldType);
					...
					if (TryEvaluateExpression(exprText, out string result, verbose)) {
						if (Config.GetValue(RoundResults) && isIntField) {
							if (double.TryParse(result, NumberStyles.Float, CultureInfo.InvariantCulture, out double value)) {
								double rounded = Math.Round(value, MidpointRounding.AwayFromZero);
								if (verbose) Msg($"Rounded {value} to {rounded} (int field)");
								result = rounded.ToString(CultureInfo.InvariantCulture);
							}
						} else if (Config.GetValue(RoundResults) && !isIntField && verbose) {
							Msg("Rounding skipped (not an int field - only int fields are rounded)");
						}
						...
						if (double.TryParse(exprText, NumberStyles.Float, CultureInfo.InvariantCulture, out double numValue)) {
							...
							if (Config.GetValue(RoundResults) && isIntField) {
								...
								if (verbose) Msg($"Rounded {numValue} to {rounded} (int field)");
								result = rounded.ToString(CultureInfo.InvariantCulture);
							}
			...
			private static Type GetFieldType(TextEditor editor, bool verbose) {
				try {
					...
					// Fallback: check editor's slot hierarchy - JUSTTT in-case all the above fails. (which, I doubt it wont, but hey, safety first!)
